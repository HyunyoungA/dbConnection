...
const mysql = require('mysql'); //db연결하고 sql쿼리돌려서 원하는 db 빼오기위해 필요
const bodyParser = require('body-parser'); // req.body 파싱하려고 , node 16버전에서는 express로 편입 호출필요없음
...
mybatisMapper.createMapper(['./mapper/reactSQL.xml']) // 다양한 플랫폼에서 쓸 수 있는 xml파일 **서버기준으로 경로연결

var format = { language : 'sql', indent : '  '}
// 노드가 쓸때 xml에 담아둔 정보가 sql언어이며, 띄어쓰기하기(구분자역할)
...
// createPool(디비접속) : 서버접속 요청에 일일이 접속하면 자원낭비 /풀장처럼 담아서 실행 =>동시에 사용할 수 있는 사용자를 담아둠.
// 1. MySQL 모듈을 불러온다.
// 2. DB와 연결한다
// 3. 쿼리를 실행한다
